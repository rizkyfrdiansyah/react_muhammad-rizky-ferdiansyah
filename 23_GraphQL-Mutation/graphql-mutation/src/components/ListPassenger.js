  ...
  return (
    <div>
      ...
      <table cellPadding="5px" cellSpacing="0" style={{ margin: "auto" }}>
        ...
        {dataPassID
          ? dataPassID?.passengers_Data_Passengers.map((item) => <ListItem deletePassanger={deletePassangers} key={item.id} data={item} />)
          : data?.passengers_Data_Passengers.map((item) => <ListItem deletePassanger={deletePassangers} key={item.id} data={item} />)}
      </table>
    </div>
  );
}
